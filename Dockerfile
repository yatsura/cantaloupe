...
COPY dist/deps/Linux-x86-64/lib /usr/local/lib
COPY --from=build /build/target/cantaloupe-5.0.7.jar /opt/cantaloupe/
ENV \
...
ENTRYPOINT [ "java", "-Dcantaloupe.config=cantaloupe.properties", "-Djava.library.path=/usr/local/lib", "-cp target/cantaloupe-5.0.7.jar", "edu.illinois.library.cantaloupe.StandaloneEntry" ]